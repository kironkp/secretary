...
describe("sweepUnderstanding", () => {
  ...
  it("(1) runs both active projects of the user and writes two records", async () => {
    ...
    expect(result).toEqual({ users: 1, ran: 2, skipped: 0, failed: 0, retiredAsr: 0, healed: 0, repaired: 0, restored: 0 });
  ...
  it("(2) the same data again is skipped on the hash and the model is not called", async () => {
    const result = await sweepUnderstanding({ now: NOW, model, userIds: [U.id] });
    expect(result).toEqual({ users: 1, ran: 0, skipped: 2, failed: 0, retiredAsr: 0, healed: 0, repaired: 0, restored: 0 });
    expect(model.calls).toHaveLength(2);
  ...
  it("(3) UNDERSTANDING_DISABLED returns zeros without reading anything", async () => {
    ...
    expect(result).toEqual({ users: 0, ran: 0, skipped: 0, failed: 0, retiredAsr: 0, healed: 0, repaired: 0, restored: 0 });
    expect(model.calls).toHaveLength(2);
  });
  ...
  it("(4) a second sweep while one is in flight returns zeros at once", async () => {
    ...
    expect(first).toEqual({ users: 1, ran: 2, skipped: 0, failed: 0, retiredAsr: 0, healed: 0, repaired: 0, restored: 0 });
  ...
  it("(6) a user with no model available still gets the ASR retire, and no run rows", async () => {
    ...
    expect(result).toEqual({ users: 1, ran: 0, skipped: 0, failed: 0, retiredAsr: 1, healed: 0, repaired: 0, restored: 0 });
