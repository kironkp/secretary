...
export const toolSchemas = {
  ...
  answer_question: z.object({
    ...
    answer_id: z
      ...
      .max(40)
      .optional()
      .describe("The answer id (before the =) whose label matches what the user said; leave it out when nothing matches and give own_words instead"),
    note: z
      .string()
      .max(500)
      .optional()
      .describe("Anything extra the user said with the answer, in their words"),
    // Flat and optional on purpose: the Realtime API takes no unions, so
    // "pick an answer" and "answer in your own words" are two optional
    // strings on one tool rather than two shapes.
    own_words: z
      .string()
      .max(1000)
      .optional()
      .describe("What the user said when it matches none of the listed answers, in their words; the app reads it against the question"),
  }),
